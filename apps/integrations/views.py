"""DRF API views for the integrations layer.

All endpoints require TokenAuthentication. The token is passed as:
    Authorization: Token <token>

Generate tokens via the management command:
...
class PostListCreateView(generics.ListCreateAPIView):
    """GET /api/v1/posts/ — list;  POST /api/v1/posts/ — create."""
    ...
    def create(self, request, *args, **kwargs):
        """Create a Post, then return it serialized as a full Post (not input fields)."""
    ...
    def get_queryset(self):
        """Return posts scoped to the user's workspaces."""
        ...
        qs = Post.objects.filter(workspace_id__in=workspace_ids)
        ...
        if platform:
            qs = qs.filter(
                platform_posts__social_account__platform=platform
            ).distinct()
...
class PostDetailView(generics.RetrieveUpdateAPIView):
    """GET /api/v1/posts/<id>/ — detail;  PATCH /api/v1/posts/<id>/ — update."""
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(workspace_id__in=workspace_ids)

    def patch(self, request, *args, **kwargs):
        """Partial update: update caption, title, scheduled_at, tags."""
...
class UnsplashSearchView(APIView):
    ...
    def get(self, request):
        from .services.unsplash import UnsplashClient, UnsplashError
        ...
        if not client.is_configured:
            return Response(
                {"error": "Unsplash API key is not configured."}, status=503
            )
...
class UnsplashImportView(APIView):
    ...
    def post(self, request):
        ...
        existing = MediaAsset.objects.filter(
            workspace=workspace, external_id=photo_id
        ).first()
        ...
        workspace_key = workspace.integration_settings.get("unsplash_access_key", "")
        client = UnsplashClient(workspace_key=workspace_key)
        if not client.is_configured:
            return Response(
                {"error": "Unsplash API key is not configured."}, status=503
        ...
        response_serializer = MediaAssetImportResponseSerializer(
...
class FolderListCreateView(APIView):
    ...
    def get(self, request):
        ...
        folders = (
            MediaFolder.objects.filter(workspace_id=workspace_id)
